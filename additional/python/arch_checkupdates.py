...
def arch_checkupdates():
    ...
    # mkdtemp, not a fixed name: under pkexec this runs as root, and a
    # pre-created /tmp/la-checkup-db-0 containing a planted `local` symlink
    # would steer root's pacman database writes to attacker-chosen paths.
    chkupdates_db = tempfile.mkdtemp(prefix='la-checkup-db-')
    try:
        ...
        return jessentials.run_command(f'/usr/bin/pacman -Qu --dbpath {chkupdates_db}', print_output=False, return_output=True)
    finally:
        shutil.rmtree(chkupdates_db, ignore_errors=True)
